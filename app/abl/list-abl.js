...
class ListAbl {
  ...
  async delete(uri, dtoIn, uuAppErrorMap = {}) {
    ...
    let uuItem = await this.itemDao.listByListIdAndState(uuObject);
    if (uuItem.forceDelete === false) {
      throw new Errors.Delete.ListContainsActiveItems({ uuAppErrorMap }, { id: dtoIn.id, itemList: item.itemList });
    }

    //HDS System deletes all item uuObjects in the list from uuAppObjectStore (using item DAO deleteManyByListId with awid and dtoIn.id).
